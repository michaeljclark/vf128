...
static int format_byte(char *buf, size_t buflen, uint8_t c)
{
...
int format_vf128(char *buf, size_t buflen, uint8_t v)
{
    float f;
    char s[16] = { 0 };
    int n = 0;
    vf_buf *vfbuf = vf_buf_new(1);
    vf_buf_write_bytes(vfbuf, (const char*)&v, 1);
    vf_buf_reset(vfbuf);
    vf_f32_read(vfbuf, &f);
    format_byte(s, sizeof(s), v);
    if (isnan(f)) {
...
void print_table(int ncols)
{
    ...
    for (uint8_t c = 0; c < ncols; c++) {
        for (uint8_t r = 0; r < nrows; r++) {
            uint8_t v = c*nrows + r;
            int n = format_vf128(buf, sizeof(buf), v);
